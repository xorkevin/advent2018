...
takes a turn, resolving all of its actions before the next unit's turn begins.
...
a wall or another unit. Alternatively, the unit might already be in range of
...
left, or right) open (`.`) square. Units cannot move into walls or other units.
...
cannot reach (find an open path to) any of the squares that are in range, it
